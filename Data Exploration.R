...
re_assessments_2015_plus = re_assessments_date %>%
  filter(SALEDATE_YEAR >= 2015, SALEDATE_YEAR < 2021)

nrow(re_assessments_2015_plus) / nrow(re_assessments_date)
#About 25% of assessment data is represented in this analysis.

#County Type of Sale

#Count how often appeals occured in time period we're looking at
...
#Grouping Test to Understand how we can filter this on single family homes bought by non-corporations.
grouped_data = re_sales_and_appeals %>%
  group_by(OWNERDESC, CLASSDESC, SALEDESC, USEDESC) %>%
  summarise(count = n())
...
appeal_rate_by_sale_year

#Pittsburgh Test
#Confirm that regex "PITTSBURGH" catches city of Pittsburgh municipality.
...
agg_test = re_sales_and_appeals %>%
  ...
  select(assessed, PARID, MUNICODE, MUNIDESC, SCHOOL_CODE, SCHOOLDESC, COMPLAINANT,  PROPERTYHOUSENUM, PROPERTYADDRESS, SALEDATE_NEW, 
         ...
         FAIRMARKETTOTAL, `PRE APPEAL TOTAL`, 
         `POST APPEAL TOTAL`, `HEARING CHANGE AMOUNT`, `HEARING DATE`) %>%
  mutate(appeal_type = case_when( #Logic to indicate what type of appeal occurred
    `HEARING DATE` <= PREVSALEDATE_NEW2 ~ 'NA-Prior_Sale_Hearing', 
    `HEARING DATE` > PREVSALEDATE_NEW2 & `HEARING DATE` <= PREVSALEDATE_NEW ~ '2_Sales_Ago', 
    `HEARING DATE` > PREVSALEDATE_NEW & `HEARING DATE` <= SALEDATE_NEW ~ '1_Sale_Ago',
    `HEARING DATE` > SALEDATE_NEW ~ 'Last_Sale',
  ...
  mutate(price_diff_purchase = case_when( #Which price difference calculation do we make based on appeal_type
    appeal_type == 'NA-No_Assessment' ~ SALEPRICE - PREVSALEPRICE,
    appeal_type == '1_Sale_Ago' ~ PREVSALEPRICE - PREVSALEPRICE2, 
    appeal_type == 'Last_Sale' ~ SALEPRICE - PREVSALEPRICE
  )) %>% 
  mutate(relevant_date = case_when( #Record relevant sale date that potentially triggered assessment.
...
complainant_types # Most complaints come from school districts after removing owner complaints
#Even with owners, they appealed ~3k times vs. ~21k for districts
...
hist(avg_assessment_lag_no_outliers$day_diff)

boxplot(avg_assessment_lag_no_outliers$day_diff)
...
final_predictors = agg_test %>%
  ...
  select(MUNIDESC, SCHOOLDESC, SALEPRICE, SALEPRICE_MINUS_ASSESSED, RELEVANT_YEAR, assessed) %>%
  mutate(RELEVANT_YEAR = as.factor(RELEVANT_YEAR))

#quick test

model_glm = glm(formula = assessed ~ MUNIDESC + SCHOOLDESC + SALEPRICE + SALEPRICE_MINUS_ASSESSED, family = "binomial", data = final_predictors)
...
model_glm_2 = glm(formula = assessed ~ SCHOOLDESC + SALEPRICE + SALEPRICE_MINUS_ASSESSED, family = "binomial", data = final_predictors)
...
model_glm_3 = glm(formula = assessed ~ SCHOOLDESC + SALEPRICE + SALEPRICE_MINUS_ASSESSED, family = "binomial", data = train)
...
model_glm_4 = glm(formula = assessed ~ SCHOOLDESC + SALEPRICE + SALEPRICE_MINUS_ASSESSED + RELEVANT_YEAR, family = "binomial", data = train)
